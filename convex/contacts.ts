...
export const getAllContacts = query({
  ...
  personalExpenses: Doc<"expenses">[];
    ...
    .collect();
  const expensesNotPaidByYou: Doc<"expenses">[] = (await ctx.db
    .query("expenses")
    .withIndex("by_group", (q)=>
    q.eq("groupId",undefined)
  )
    .collect()).filter(e=>e.paidByUserId !== currentUser._id && e.splits
    .some(s=>s.userId==currentUser._id)); 
...
personalExpenses.forEach((exp)=> {
  if(exp.paidByUserId !== currentUser._id)
    contactsIds.add(exp.paidByUserId);
